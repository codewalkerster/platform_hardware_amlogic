...
std::map<std::string, SensorInfo> ParseSensorInfo(std::string_view config_path) {
    ...
    std::ifstream json(config_path);
    if (!Json::parseFromStream(builder, json, &root, &error_message)) {
        LOG(ERROR) << "Failed to parse JSON config. " << error_message;
