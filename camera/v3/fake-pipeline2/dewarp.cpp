...
namespace android {
    ...
    int DeWarp::dewarp_init() {
        CameraConfig* config = CameraConfig::getInstance(mGroupId);
        CAMHAL_LOGD("E in %dx%d out %dx%d \n", config->getInputWidth(), config->getInputHeight(), config->getOutputWidth(), config->getOutputHeight());
        ...
        if (mGroupId <= DEWARP_CAM2PORT_DPTZ_PREVIEW) {
            ...
            in_params.o_width = config->getOutputWidth();
            in_params.o_height = config->getOutputHeight();
            in_params.i_width = (config->getInputWidth() > 0) ? config->getInputWidth() : config->getOutputWidth();
            in_params.i_height = (config->getInputHeight() > 0) ? config->getInputHeight() : config->getOutputHeight();
            in_params.rotation = mRotation;
            ret = staticPipe::fetchSensorGdcParameter(
                            &config->mSensorParam, in_params, &mDewarp_params);
            if (ret != 0)
                CAMHAL_LOGE("fetch sensor fail, ret %d", ret);
        } else {
            ...
            */
            ...
            int width_tmp = config->getOutputWidth();
            int height_tmp = config->getOutputHeight();
    ...
    int DeWarp::gdc_init() {
        ...
        mGDCContext->plane_number = config->mGDCParam.planeNum;   /* data in one continuous mem block */
        ...
        if (gdc_format == NV12) {
            ...
            o_y_stride = (config->mGDCParam.o_stride > 0) ? config->mGDCParam.o_stride : AXI_WORD_ALIGN(o_width);
            i_c_stride = AXI_WORD_ALIGN(i_width);
            o_c_stride = (config->mGDCParam.o_stride > 0) ? config->mGDCParam.o_stride : AXI_WORD_ALIGN(o_width);
