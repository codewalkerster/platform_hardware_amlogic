...
int GlobalResource::getFd(char* device_name, int id, std::vector<int> &fds) {
    ...
    for (int i = 0; i < HAL_MAX_FD_NUM; i++) {
        CAMHAL_LOGD("%s: fd %d", __FUNCTION__, mDeviceFd[id][i]);
        if (mDeviceFd[id][i] == -1) {
            IsInit = false;
            break;
        }
    }
    CAMHAL_LOGD("%s: id %d init? %d E", __FUNCTION__, id, IsInit);
    if (!IsInit) {
        CAMHAL_LOGD("%s:init global resource , name:%s", __FUNCTION__, device_name);
        for (int i = 0; i < HAL_MAX_FD_NUM; i++)
