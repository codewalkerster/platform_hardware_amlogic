...
#include "hi556_sdr_calibration.h"
#include "hi556_wdr_calibration.h"
#include "hi556_api.h"
...
void cmos_set_sensor_entity_hi556(int ViPipe, struct media_entity * sensor_ent, int wdr, int fps)
{
...
void cmos_get_sensor_gdc_parameter_hi556(struct sensorConfig *cfg, GDCInParam in_params,
...
#endif
void cmos_get_sensor_calibration_hi556(int ViPipe, struct media_entity *sensor_ent, aisp_calib_info_t *calib)
{
    if (g_sensorPtr[ViPipe]->enWDRMode == 1)
        Hi556WdrCalibration::dynamic_wdr_calibrations_init_hi556(calib);
    else
        Hi556SdrCalibration::dynamic_sdr_calibrations_init_hi556(calib);
}

void cmos_clean_up_hi556(int ViPipe)
{
    if (cam_id_check(ViPipe)) {
        CAMHAL_LOGE("invalid id %d", ViPipe);
        return;
...
int cmos_get_ae_default_hi556(int ViPipe, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 2816; //hts
    g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 1988; //vts
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.again_log2_max = 17203; // 2^4.2 = 18x
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2_max = 17203;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = (0x4c7<<(SHUTTER_TIME_SHIFT));
...
void cmos_again_calc_table_hi556(int ViPipe, uint32_t *pu32AgainLin, uint32_t *pu32AgainDb)
{
    ...
    again_reg = aisp_math_exp2( *pu32AgainLin, SHUTTER_TIME_SHIFT, 6 );
...
void cmos_dgain_calc_table_hi556(int ViPipe, uint32_t *pu32DgainLin, uint32_t *pu32DgainDb)
{
    //CAMHAL_LOGD("cmos_dgain_calc_table: %d, %d\n", *pu32DgainLin, *pu32DgainDb);
}

void cmos_inttime_calc_table_hi556(int ViPipe, uint32_t pu32ExpL, uint32_t pu32ExpS, uint32_t pu32ExpVS, uint32_t pu32ExpVVS)
{
    ...
    if (g_sensorPtr[ViPipe]->enWDRMode == 0) {
        if (shutter_time_lines > (g_sensorPtr[ViPipe]->snsAlgInfo.total.height-2))
            shutter_time_lines = g_sensorPtr[ViPipe]->snsAlgInfo.total.height-2;
...
void cmos_fps_set_hi556(int ViPipe, float f32Fps, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    CAMHAL_LOGD("-hi556- f32Fps = %f, %d\n",f32Fps, (int32_t)(f32Fps / 256));
    uint32_t clk_cnt;
    ...
    CAMHAL_LOGD("-hi556- setting fps = %d\n", sensorCtrl.value);
    ...
    CAMHAL_LOGD("-hi556- vtsCtrl.value = %d\n", sensorCtrl.value);
    v4l2_subdev_set_ctrls(g_sensorPtr[ViPipe]->sensor_ent, &sensorCtrl, 1);
...
void cmos_alg_update_hi556(int ViPipe)
{
    ...
    if ( g_sensorPtr[ViPipe]->snsAlgInfo.u16GainCnt || g_sensorPtr[ViPipe]->snsAlgInfo.u16IntTimeCnt ) {
        ...
        if ( g_sensorPtr[ViPipe]->snsAlgInfo.u16IntTimeCnt ) {
            ...
            if (g_sensorPtr[ViPipe]->enWDRMode) {
                ...
                //hi556_write_register(ViPipe, 0x3020, shutter_time_lines_short & 0xff);
                //hi556_write_register(ViPipe, 0x3021, (shutter_time_lines_short>>8) & 0xff);
                //hi556_write_register(ViPipe, 0x3024, shutter_time_lines&0xff);
                //hi556_write_register(ViPipe, 0x3025, (shutter_time_lines>>8) & 0xff);
