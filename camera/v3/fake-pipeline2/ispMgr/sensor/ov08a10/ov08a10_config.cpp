...
void cmos_set_sensor_entity_ov08a10(int ViPipe, struct media_entity * sensor_ent, int wdr, int fps)
{
...
void cmos_get_sensor_calibration_ov08a10(int ViPipe, struct media_entity *sensor_ent, aisp_calib_info_t *calib)
{
...
void cmos_get_sensor_gdc_parameter_ov08a10(struct sensorConfig *cfg, GDCInParam in_params,
...
void cmos_clean_up_ov08a10(int ViPipe)
{
    if (cam_id_check(ViPipe)) {
        CAMHAL_LOGE("invalid id %d", ViPipe);
        return;
...
int cmos_get_ae_default_ov08a10(int ViPipe, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.active.width = 3840;
    g_sensorPtr[ViPipe]->snsAlgInfo.active.height = 2160;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 2072; //hts
    g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 2314; //vts

    g_sensorPtr[ViPipe]->snsAlgInfo.lines_per_second = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 4)* g_sensorPtr[ViPipe]->snsAlgInfo.fps / 256;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.again_log2_max = (1984)<<(LOG2_GAIN_SHIFT); //15.5x128=1984
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2_max = (1984)<<(LOG2_GAIN_SHIFT);
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = (0x6cd<<(SHUTTER_TIME_SHIFT));
...
void cmos_fps_set_ov08a10(int ViPipe, float f32Fps, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    //CAMHAL_LOGD("--ov08a10-- fpsCtrl.value = %d\n",fpsCtrl.value);

    g_sensorPtr[ViPipe]->snsAlgInfo.total.height = ( 2314 * 30 )/fpsCtrl.value;

    g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_max = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 4 ) << SHUTTER_TIME_SHIFT;
    g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_long_max = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 4 ) << SHUTTER_TIME_SHIFT;
    g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_limit = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 4 ) << SHUTTER_TIME_SHIFT;
    g_sensorPtr[ViPipe]->snsAlgInfo.lines_per_second = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height -4) * fpsCtrl.value;
    memcpy(pstAeSnsDft, &g_sensorPtr[ViPipe]->snsAlgInfo, sizeof(ALG_SENSOR_DEFAULT_S));
