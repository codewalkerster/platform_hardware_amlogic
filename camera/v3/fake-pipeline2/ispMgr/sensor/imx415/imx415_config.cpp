...
void cmos_set_sensor_entity_imx415(int ViPipe, struct media_entity * sensor_ent,int wdr, int fps)
{
...
void cmos_get_sensor_calibration_imx415(int ViPipe, struct media_entity *sensor_ent, aisp_calib_info_t *calib)
{
...
void cmos_clean_up_imx415(int ViPipe)
{
    if (cam_id_check(ViPipe)) {
        CAMHAL_LOGE("invalid id %d", ViPipe);
        return;
...
int cmos_get_ae_default_imx415(int ViPipe, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    if (g_sensorPtr[ViPipe]->enWDRMode == 1) {
        ...
        g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 0x0215;   // sync with reg HMAX
        g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 0x08D0;  // sync with reg VMAX
        ...
        g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 0x042A;   // sync with HMAX
        g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 0x08D0;  // sync with reg VMAX
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.again_log2_max = (48/6)<<(LOG2_GAIN_SHIFT);
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2_max = (48/6)<<(LOG2_GAIN_SHIFT);
    ...
    if (g_sensorPtr[ViPipe]->enWDRMode == 1) {
        ...
        // long exposure time = 2*vmax - SHR0
        g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = ((2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 0x105c)<<(SHUTTER_TIME_SHIFT));

        // short exposure time. calc from initial value of RSH1 - SHR1
        g_sensorPtr[ViPipe]->snsAlgInfo.sexpos_lines = (0x111 - 0x9)<< SHUTTER_TIME_SHIFT;

    } else {
        // sdr mode
        // long exposure time = vmax - SHR0; in SDR
        g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 0x08) << SHUTTER_TIME_SHIFT;
