...
#include "ov08d10_sdr_calibration.h"
#include "ov08d10_wdr_calibration.h"
#include "ov08d10_api.h"
...
void cmos_set_sensor_entity_ov08d10(int ViPipe, struct media_entity * sensor_ent,int wdr, int fps)
{
...
void cmos_get_sensor_gdc_parameter_ov08d10(struct sensorConfig *cfg, GDCInParam in_params,
...
void cmos_get_sensor_calibration_ov08d10(int ViPipe, struct media_entity *sensor_ent, aisp_calib_info_t *calib)
{
    if (g_sensorPtr[ViPipe]->enWDRMode == 1)
        Ov08d10WdrCalibration::dynamic_wdr_calibrations_init_ov08d10(calib);
    else
        Ov08d10SdrCalibration::dynamic_sdr_calibrations_init_ov08d10(calib);
}

void cmos_clean_up_ov08d10(int ViPipe)
{
    if (cam_id_check(ViPipe)) {
        CAMHAL_LOGE("invalid id %d", ViPipe);
        return;
...
int cmos_get_ae_default_ov08d10(int ViPipe, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    CAMHAL_LOGD("cmos_get_ae_default\n");

    g_sensorPtr[ViPipe]->snsAlgInfo.active.width = 3264;
    g_sensorPtr[ViPipe]->snsAlgInfo.active.height = 2448;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 460; //hts 1cc
    g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 2504; //vts 2504, sensor vmax register[0x34-0x35, 0x36]

    g_sensorPtr[ViPipe]->snsAlgInfo.lines_per_second = 2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height*30;
    ...
    if (g_sensorPtr[ViPipe]->enWDRMode == 1) {
        ...
        g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_max = (2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height-20) << SHUTTER_TIME_SHIFT;
        g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_long_max = (2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height-20) << SHUTTER_TIME_SHIFT;
        g_sensorPtr[ViPipe]->snsAlgInfo.integration_time_limit = (2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height-20) << SHUTTER_TIME_SHIFT;
    }

    g_sensorPtr[ViPipe]->snsAlgInfo.again_log2_max = 16384; //2^4 = 16x
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2_max = 16384;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2 = 0x0 << LOG2_GAIN_SHIFT;
    g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = (0x137c << (SHUTTER_TIME_SHIFT));
...
void cmos_again_calc_table_ov08d10(int ViPipe, uint32_t *pu32AgainLin, uint32_t *pu32AgainDb)
{
    uint32_t again_reg;
    again_reg = aisp_math_exp2( *pu32AgainLin, SHUTTER_TIME_SHIFT, 4 );
    if (again_reg > 0xf8) {
        again_reg = 0xf8;
    }
    //CAMHAL_LOGD("again_reg2: %d\n", again_reg);
...
void cmos_dgain_calc_table_ov08d10(int ViPipe, uint32_t *pu32DgainLin, uint32_t *pu32DgainDb)
{
    //CAMHAL_LOGD("cmos_dgain_calc_table: %d, %d\n", *pu32DgainLin, *pu32DgainDb);
}

void cmos_inttime_calc_table_ov08d10(int ViPipe, uint32_t pu32ExpL, uint32_t pu32ExpS, uint32_t pu32ExpVS, uint32_t pu32ExpVVS)
{
    ...
    if (g_sensorPtr[ViPipe]->enWDRMode == 0) {
        if (shutter_time_lines > (2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height - 20))
            shutter_time_lines = 2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height-20;
...
void cmos_fps_set_ov08d10(int ViPipe, float f32Fps, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    CAMHAL_LOGD("-ov08d10- f32Fps = %f, %d\n",f32Fps, (int32_t)(f32Fps / 256));
    uint32_t clk_cnt;
    struct v4l2_ext_control sensorCtrl;
    uint32_t original_vmax = 2504;
    ...
    CAMHAL_LOGD("-ov08d10- setting fps = %d\n", sensorCtrl.value);
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.lines_per_second =
        2 * g_sensorPtr[ViPipe]->snsAlgInfo.total.height * sensorCtrl.value;

    memset(&sensorCtrl, 0, sizeof(struct v4l2_ext_control));
    sensorCtrl.id = V4L2_CID_AML_VTS;
    sensorCtrl.value = 2 * (g_sensorPtr[ViPipe]->snsAlgInfo.total.height - original_vmax);
    CAMHAL_LOGD("-ov08d10- vtsCtrl.value = %d\n", sensorCtrl.value);
    v4l2_subdev_set_ctrls(g_sensorPtr[ViPipe]->sensor_ent, &sensorCtrl, 1);
...
void cmos_alg_update_ov08d10(int ViPipe)
{
    ...
    if ( g_sensorPtr[ViPipe]->snsAlgInfo.u16GainCnt || g_sensorPtr[ViPipe]->snsAlgInfo.u16IntTimeCnt ) {
        if ( g_sensorPtr[ViPipe]->snsAlgInfo.u16GainCnt ) {
            ...
            CAMHAL_LOGD("gain value = %d",gain.value);
        ...
        if ( g_sensorPtr[ViPipe]->snsAlgInfo.u16IntTimeCnt ) {
            ...
            if (g_sensorPtr[ViPipe]->enWDRMode == 0) {
                ...
                CAMHAL_LOGD("expo.value = %d",expo.value);
            ...
            if (g_sensorPtr[ViPipe]->enWDRMode) {
                ...
                //ov08d10_write_register(ViPipe, 0x3020, shutter_time_lines_short & 0xff);
                //ov08d10_write_register(ViPipe, 0x3021, (shutter_time_lines_short>>8) & 0xff);
                //ov08d10_write_register(ViPipe, 0x3024, shutter_time_lines&0xff);
                //ov08d10_write_register(ViPipe, 0x3025, (shutter_time_lines>>8) & 0xff);
