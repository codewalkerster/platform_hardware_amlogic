...
void cmos_set_sensor_entity_ov13855(int ViPipe, struct media_entity * sensor_ent, int wdr, int fps)
{
...
void cmos_get_sensor_calibration_ov13855(int ViPipe, struct media_entity * sensor_ent, aisp_calib_info_t * calib)
{
...
void cmos_clean_up_ov13855(int ViPipe)
{
    if (cam_id_check(ViPipe)) {
        CAMHAL_LOGE("invalid id %d", ViPipe);
        return;
...
int cmos_get_ae_default_ov13855(int ViPipe, ALG_SENSOR_DEFAULT_S *pstAeSnsDft)
{
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.active.width = 4224;
    g_sensorPtr[ViPipe]->snsAlgInfo.active.height = 3136;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.total.width = 0x462;
    g_sensorPtr[ViPipe]->snsAlgInfo.total.height = 0xCA0;
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.again_log2_max = 1984 << SHUTTER_TIME_SHIFT;
    g_sensorPtr[ViPipe]->snsAlgInfo.again_high_log2_max = 1984 << SHUTTER_TIME_SHIFT; //15.5x128 = 1984
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.expos_lines = (0x2A2<<(LOG2_GAIN_SHIFT));
...
{
    CAMHAL_LOGD("-ov13855- f32Fps = %f, %d\n",f32Fps, (int32_t)(f32Fps / 256));
    uint32_t clk_cnt;
    ...
    CAMHAL_LOGD("-ov13855- setting fps = %d\n", sensorCtrl.value);
    ...
    g_sensorPtr[ViPipe]->snsAlgInfo.lines_per_second =
        (g_sensorPtr[ViPipe]->snsAlgInfo.total.height-8) * sensorCtrl.value;

    memset(&sensorCtrl, 0, sizeof(struct v4l2_ext_control));
    sensorCtrl.id = V4L2_CID_AML_VTS;
    sensorCtrl.value = g_sensorPtr[ViPipe]->snsAlgInfo.total.height;
    CAMHAL_LOGD("-ov13855- vtsCtrl.value = %d\n", sensorCtrl.value);
    v4l2_subdev_set_ctrls(g_sensorPtr[ViPipe]->sensor_ent, &sensorCtrl, 1);
